...
namespace eval ::TE {
  ...
  namespace eval VLAB {
   # -----------------------------------------------------------------------------------------------------------------------------------------
    ...
    proc hw_program_flash {{appname ""} {fname ""} {setup 011001} {erase false}} {
      ...
      if {[string match -nocase *xc*7z* $hw_fpga_name] } {
        #zynq
          ...
          if {$erase} {
            ...
            if {![file exists ${binfilename}]} {
              ...
              puts $fp_w "Dummy files for erase flash only option"
              close $fp_w
            }
          } elseif {$appname eq "" } {
            if {[catch {set binfilename [glob -join -dir ${TE::BASEFOLDER}/ *.bin]}] } {
            ...
            if {[catch {set fsbl [glob -join -dir ${TE::BASEFOLDER}/ fsbl_flash.elf]}] } {
               TE::UTILS::te_msg TE_PR-98 {CRITICAL WARNING} "*.bin is not available on ${TE::BASEFOLDER}/"
                set fsbl ""
            }
        ...
        #zynqmp
          ...
          set_property PROGRAM.FILES [list "${binfilename}" ] [ get_property PROGRAM.HW_CFGMEM [lindex [get_hw_devices $hw_fpga_name] 0]]    
          set_property PROGRAM.ZYNQ_FSBL ${fsbl} [ get_property PROGRAM.HW_CFGMEM [lindex [get_hw_devices $hw_fpga_name] 0]]      
      } elseif {[string match -nocase *xc*7* $hw_fpga_name] }  {
        #7series fpga
          ...
          set prmfilename ""
          if {$erase} {
            set prmfilename ${TE::TMP_PATH}/erase_flash.mcs
            if {![file exists ${binfilename}]} {
              file mkdir ${TE::TMP_PATH}/
              set fp_w [open ${binfilename} "w"]
              puts $fp_w "Dummy files for erase flash only option"
            ...
            if {[catch {set mcsfilename [glob -join -dir ${TE::BASEFOLDER}/ *.bin]}] } {
               TE::UTILS::te_msg TE_PR-101 {CRITICAL WARNING} "*.bin is not available on ${TE::BASEFOLDER}/"
                set mcsfilename ""
            }
            if { ![catch {set prmfilename [glob -join -dir ${TE::BASEFOLDER}/ *.prm]}]} {

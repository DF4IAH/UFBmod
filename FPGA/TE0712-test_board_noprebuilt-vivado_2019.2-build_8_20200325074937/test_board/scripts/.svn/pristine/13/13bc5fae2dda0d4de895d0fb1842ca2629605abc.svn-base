...
namespace eval ::TE {
  ...
  # -----------------------------------
  # check csv file ids
  variable SCRIPTVER    "2019.2.8"
  variable BOARDDEF_CSV "1.4"
  variable SW_IP_CSV    "2.3"
        ...
        foreach line $data {
          ...
          if {[string match *#* $line] != 1 && [string match *CSV_VERSION* $line] } {
            ...
            if {[string match [lindex $tmp 1] $TE::SW_IP_CSV] != 1} {
              ...
              TE::UTILS::te_msg TE_INIT-?? INFO "Software Definition CSV version passed"
            }
          } elseif {[string match *#* $line] != 1 && [string length $line] > 0} {
            if {$version_check eq false} {
              TE::UTILS::te_msg TE_INIT-?? ERROR "Software Definition CSV Version check was not done."
              ...
              if {$table eq "platform"} {
                ...
                if {[string match "id" [lindex $tmp 0]] } {
                  TE::UTILS::te_msg TE_INIT-?? INFO "Software Definition CSV Version analyze platform table header"
                  ...
                  for {set i 0} {$i < $element_length} {incr i} {
                    ...
                    switch $element {
                      ...
                      "name" {
                        set TE::SDEF::P_NAME $i
                      }
                      "proc" {
                        set TE::SDEF::P_PROC $i
                      }
                      "os" {
                        set TE::SDEF::P_OS $i
                      }
                  ...
                  # append
                  lappend TE::SDEF::SW_PLATLIST $tmp
                ...
                #header
                if {[string match "id" [lindex $tmp 0]] } {
                  TE::UTILS::te_msg TE_INIT-?? INFO "Software Definition CSV Version analyze bsp table header"
                  set element NA
                  for {set i 0} {$i < $element_length} {incr i} {
                    set element [lindex $tmp $i]
                    switch $element {
                ...
                #header
                if {[string match "id" [lindex $tmp 0]] } {
                  TE::UTILS::te_msg TE_INIT-?? INFO "Software Definition CSV Version analyze app table header"
                  set element NA
                  for {set i 0} {$i < $element_length} {incr i} {
                    set element [lindex $tmp $i]
                    switch $element {
                ...
                TE::UTILS::te_msg TE_INIT-?? {CRITICAL WARNING} "Software Definition CSV unknown definition: $table $tmp."
              }
            }
          }
        }
  ...
  # -----------------------------------------------------------------------------------------------------------------------------------------
    ...
    set BUILD                         6
    set CSYMB                         7
    set DESTINATION_CPU               9
    
    set ZYNQMP_FSBL_CONFIG            8
    set ZYNQMP_EXCEPTION_LEVEL        10
    set ZYNQMP_ATF                    11
    set ZYNQMP_PMU                    12
    set ZYNQMP_DATA01_FILE            13
    set ZYNQMP_DATA01_LOAD            14
    set ZYNQMP_DATA01_OFFSET          15
    #...ZYNQMP only 01 possible at the moment
    set MB_DATA01_FILE     9
    set MB_DATA01_LOAD     10
    set MB_DATA01_OFFSET   11
    #...MB multiple files possible 02 (10...12), 03(13..15)
    set ZYNQ_DATA01_FILE      9
    set ZYNQ_DATA01_LOAD      10
    set ZYNQ_DATA01_OFFSET    11
